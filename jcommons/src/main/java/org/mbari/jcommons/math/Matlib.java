...
public class Matlib {
     ...
     * @throws IllegalArgumentException
     */
    public static double[] interpolate(long[] x, double[] y, long[] xi)
            throws IllegalArgumentException {
    ...
    /**
     * Find the index of the value nearest to the key. The values array can contain only unique
     * values. If it doesn't the first occurence of a value in the values array is the one used,
     * subsequent duplicate are ignored.
     *
    ...
    /**
     * Find the index of the array nearest to the value. The values array can contain only unique
     * values. If it doesn't the first occurence of a value in the values array is the one used,
     * subsequent duplicate are ignored. If the value falls outside the bounds of the array,
     ...
     * @return The index of the array value nearest the value. -1 will be returned if the key is
     *     outside the array values
     */
     ...
     * @return The index of the array value nearest the value. null if the value is larger or
     *     smaller than any values in the array.
    ...
    /**
     * Returns an array of indices indicating the order the data should be sorted in. Duplicate
     * values are discarded with the first one being kept. This method is useful when a number of
     * data arrays have to be sorted based on the values in some coordinate array, such as time.
     *
     * <p>To convert a array of values to a sorted monooic array try: <br>
     * double[] x; // some 1-D array of data <br>
     * int[] i = Matlib.uniqueSort(x); <br>
     * double[] xSorted = Matlib.reorder(x, i);<br>
     * <br>
     *
     * @param x An array of data that is to be sorted.
     * @return order An array of indexes such that y = Array.sort(x) and y = x(order) are the same.
     */
